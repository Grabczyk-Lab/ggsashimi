...
def shrink_junctions(dons, accs, introns):
    ...
    for a, b in introns:
        length = b - a
        shift_acc += length-int(length**0.7)
            ...
            for a, b in intersected_introns:
                ...
                for i, (exon_start, exon_end, strand) in enumerate(exons.get(tx, [])):
                    ...
                    if a < exon_start:
                        if b > exon_end:
                            if i == len(exons[tx])-1:
                                total_shift = total_shift - shift + (exon_start - a)*(1-int(length**-0.3))
                            shift = (exon_start - a)*(1-int(length**-0.3))
